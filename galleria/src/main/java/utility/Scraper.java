	...
	public static void main(String[] args) {
		...
		try {
			...
			FileWriter foto_file = new FileWriter(
					new File("src/main/resources/foto.txt"));
			...
			foto_file.close();
			/*FileWriter tag_file = new FileWriter(
					new File("src/main/resources/foto.txt"));
			foto_html = Jsoup.connect("https://www.photoawards.com/categories/").get();
			Elements links = foto_html.select("body > div.wrapper > div > div > div > div.full_width > div > div.vc_row.wpb_row.section.vc_row-fluid.white-page.grid_section > div > div > div > div > div > div:nth-child(2) > div.wpb_column.vc_column_container.vc_col-sm-8 > div > div > div.wpb_text_column.wpb_content_element > div > ul");
			Set<String> tags = new HashSet<String>();
			for (Element link : links) {
				for(Element yolo: link.getElementsByTag("li")) {
					tags.add(yolo.child(0).text());
				}
			}
			for(String t: tags) {
				tag_file.write(t+"\n");
